...
def get_documentdb_collection():
    credentials = dict([s.split("=", 1) for s in current_app.config["COSMOSDB_CONNSTR"].split(";") if s != ""])
    client = document_client.DocumentClient(credentials["AccountEndpoint"], { "masterKey" : credentials["AccountKey"] })

    db = next((item for item  in client.ReadDatabases() if item ["id"] == current_app.config["COSMOSDB_DATABASE"]))
    collection = next((item for item in client.ReadCollections(db["_self"]) if item["id"] == current_app.config["COSMOSDB_COLLECTION"]))
